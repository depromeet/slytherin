...
public record StoreSearchHistoryResponse(@Schema(description = "검색어")
                                         String query,
                                         @Schema(description = "생성 시간")
                                         LocalDateTime updateAt) {

    public static StoreSearchHistoryResponse from(SearchHistory searchHistory) {
        return new StoreSearchHistoryResponse(searchHistory.getQuery(), searchHistory.getUpdatedAt());
    }
}
